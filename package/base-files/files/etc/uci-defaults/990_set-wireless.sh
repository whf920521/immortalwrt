	...
	if [[ $band == "5g" ]]; then
		if [ -z "$FIRST_5G" ]; then
			if [ $RADIO_NUM -eq 2 ]; then
				configure_wifi $1 'auto' "${BASE_SSID}-5G" '12345678'
			else
				configure_wifi $1 'auto' "${BASE_SSID}-5G_1" '12345678'
			fi
			FIRST_5G="1"
		else
			configure_wifi $1 'auto' "${BASE_SSID}-5G_2" '12345678'
		fi
	else
		configure_wifi $1 'auto' "${BASE_SSID}" '12345678'
	fi
}
